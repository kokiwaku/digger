    ...
    <ul className="topic-tree">
      {nodes.map((node) => (
        <li key={node.id} className="topic-node">
          <p className="topic-node-name">{node.name}</p>
          {node.items.length > 0 && (
          ...
          {node.children.length > 0 && (
            <div className="topic-node-children">
              <TopicTree nodes={node.children} onSelect={onSelect} />
            </div>
          )}
        </li>
      ))}
...
// 力学シミュレーション等は導入せず、node数に応じた単純な円形レイアウトで初期配置する
// （drag可能なので、ユーザーが見やすい位置に動かせる。位置の永続化はしない）。
function layoutNodesInCircle(items: SavedKnowledge[]): Node[] {
  ...
  return items.map((item, index) => {
    ...
    return {
      ...
      style: {
        border: dimmed ? "1px solid #ccc" : "1px solid #333",
        borderRadius: 8,
...
function KnowledgeMap({
  ...
  const nodes = useMemo(() => layoutNodesInCircle(items), [items]);
  const edges = useMemo(() => buildEdgesFromRelations(items), [items]);
          ...
          {tab === "topic" && (
            <div className="understanding-view">
              {(() => {
                ...
                return (
                  <>
                    <TopicTree nodes={tree} onSelect={setSelected} />
                    {unclassified.length > 0 && (
                      <div className="topic-node">
                        <p className="topic-node-name">未分類</p>
                        <ul className="topic-node-items">
