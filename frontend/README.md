...
│   ├── ClusterHaloNode.tsx # React FlowのCustom Node（Root Topicごとの島を示す、控えめな背景halo。非インタラクティブ）
...
- **`UnderstandingMapView.tsx`**: 「マップ」タブの実体。Topic/Concept/ConceptRelationモデル（backendの`GET /api/understanding-map`・`POST /api/understanding-map/refresh`）を使い、「保存したKnowledgeを並べたグラフ」ではなく「今のユーザーの理解状態」を感じられるUIを構築します。**Topic hierarchyを理解構造の背骨、ConceptRelationを理解の横方向の広がりとして扱う**設計です（完全なTreeでも完全なforce-directed graphでもない、中間的な構造を目指す）。「最近」「トピック」タブは引き続き旧`Knowledge.topicPath`モデルのまま無変更です（2つの分類システムが並存。Mapの階層は`Topic.parentId`由来のTopic/Conceptモデル側を単一の情報源として使っており、`topicPath`とは完全には一致しません。詳細は[`backend/README.md`](../backend/README.md#topic--concept--conceptrelationモデルtopicts--conceptts--conceptrelationts--understandingstructurets)を参照）。
  ...
  - **中央: マップ（`mapLayout.ts` + `TopicNode.tsx` + `ConceptNode.tsx` + `ClusterHaloNode.tsx`）**: 「Conceptをランダムに散らしてつなぐもの」ではなく、Topic hierarchyを理解構造の背骨、ConceptRelationを理解の横方向の広がりとして扱う「地図」として設計しています。以前は[`dagre`](https://github.com/dagrejs/dagre)が出した座標をそのまま最終座標として使っていましたが、「階層ごとにX座標が揃いすぎる」「siblingが等間隔すぎる」「業務フロー図・組織図に見える」という指摘を受け、dagreの出力を**そのまま使わず**、階層構造という骨格を保ったまま少し有機的に崩す`applyRelaxedLayout()`という後処理レイヤーを追加しました（詳細は次項）。
    ...
    - **relaxed tree layout（`applyRelaxedLayout()`、`mapLayout.ts`）**: dagreの出力（center座標。`computeHierarchyLayoutCenters()`）を初期値として、次の順序で少しずつ崩します。すべて**node idの文字列ハッシュに基づく決定的な処理**で、`Math.random()`は一切使いません（同じデータなら常に同じ配置になる。「整列」ボタンを何度押しても結果は変わりません）。
      1. **有機的なjitter（`applyOrganicJitter()`）**: 階層（Root Topic/Subtopic/Concept）ごとに強さを変えたjitterを、dagreのrank軸（LRなのでx＝階層方向）には小さく（6〜16px）、cross軸（y＝sibling展開方向）には大きく（14〜40px）加えます。階層方向は小さくとどめることで「どの深さにいるか」は読めるままにし、cross方向を大きくすることで「siblingが完全な等間隔」に見えないようにしています。Root Topicほど揺らぎを小さくし、「大きな中心」であることを保っています。
      2. **クラスタ（Root Topicの島）オフセット（`applyClusterOffsets()`）**: 複数のRoot Topicが「すべて」表示されるとき、dagreは非連結なtreeを単純に並べるだけで一直線に整列しがちになるため、ルートTopic idごとに決定的なY方向オフセット（最大±46px）を与え、完全な横一列ではなく緩やかに散らばった「島」に見えるようにします。
      3. **ConceptRelationによる引き寄せ（`applyRelationAttraction()`）**: ConceptRelationで結ばれたConcept同士を、3回の反復でcross方向（y）を中心に少しずつ近づけます（1回あたり最大36px）。階層方向（x）への影響は弱く抑え、Topic hierarchy上の大枠の位置（＝どの深さ・どのTopic配下か）を壊さないようにしています。
      4. **重なり解消（`resolveCollisions()`）**: jitter・引き寄せの結果node同士が重なることがあるため、円同士の距離が(半径の和+余白14px)未満なら押し離す、という単純な処理を4回反復するだけで止めます（長時間シミュレーションは行いません。数十〜100 node程度なら初期表示は十分高速です）。
      - 最終的なcenter座標は`centerToTopLeft()`でReact Flowの左上原点座標に変換されます。Topicフィルタ切り替え時も含め、`recomputeLayout()`が呼ばれるたびに必ずこの4ステップを経るため、「Topic filter時だけ完全なTreeへ戻る」ことはありません。
    - **edgeの見た目**: 「業務フロー図に見える」「AからBへ処理が流れるように見える」という指摘を受け、parent-child edge（`buildParentChildEdges()`）は以前の`smoothstep`（elbow風の折れ線）から、React Flowのデフォルトのなめらかなcurve（bezier）に変更し、**矢印（marker）も廃止しました**。Mapはprocess flowではなく、親子関係は位置関係と線だけで十分読めるという考え方です。色も`#c2c2c2`とさらに控えめにしています。cross relation edge（`buildCrossRelationEdges()`）は元々bezier曲線でしたが、矢印を10px→7pxへさらに小さくしました（薄いグレーの破線＋小さな日本語typeラベルはそのまま）。
    - **クラスタの背景halo（`ClusterHaloNode.tsx`）**: Root Topicごとの「島」の領域を、大きな枠線ではなく、中心が薄く色づき外側にいくほど透明になるradial gradientの円でなんとなく示します。最終座標（relaxed layout後）からクラスタごとのbounding boxを求め、それを覆う程度の円をzIndex最小（`-1`）で最背面に敷いています。`draggable: false`・`selectable: false`・`pointerEvents: none`で、クリック・ドラッグ・hover・選択のいずれの対象にもなりません。
    - **複数のルートTopicは独立したtreeとして横に並ぶ**: 「すべて」表示では、dagreが非連結なグラフ（複数のルートTopicにまたがるtree群）をまとめて配置し、それぞれ独立したtreeとして並びます（前述のクラスタオフセットで、完全な横一列にはなりません）。無理に1つのグラフへ繋げようとはせず、`nodesep`/`ranksep`は木同士が混ざらない程度の余白にとどめています（`ranksep: 70`/`nodesep: 36`）。
    ...
    - **User drag（session中の位置維持）**: node位置は`useNodesState`のローカルstateで管理しており、ドラッグ操作は`onNodesChange`経由でこのstateを直接更新するだけです。`recomputeLayout()`はTopicフィルタ変更・データ再取得・「整列」ボタンのときだけ呼ばれ、hoverや選択、他nodeのドラッグなどでは呼ばれないため、ユーザーがドラッグしたnodeの位置はセッション中（DB永続化はしていません）維持されます。
