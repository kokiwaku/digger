...
- **`UnderstandingMapView.tsx`**: 「マップ」タブの実体。Topic/Concept/ConceptRelationモデル（backendの`GET /api/understanding-map`・`POST /api/understanding-map/refresh`）を使い、「保存したKnowledgeを並べたグラフ」ではなく「今のユーザーの理解状態」を感じられるUIを構築します。**Root Topic → Subtopic → Concept → Knowledgeという階層構造そのものをMapの骨格にする**（Topic hierarchyを基本構造とし、ConceptRelationはそれを補足する横断的なつながりとして重ねるだけ）という設計に作り替えています。「最近」「トピック」タブは引き続き旧`Knowledge.topicPath`モデルのまま無変更です（2つの分類システムが並存。Mapの階層は`Topic.parentId`由来のTopic/Conceptモデル側を単一の情報源として使っており、`topicPath`とは完全には一致しません。詳細は[`backend/README.md`](../backend/README.md#topic--concept--conceptrelationモデルtopicts--conceptts--conceptrelationts--understandingstructurets)を参照）。
  ...
  - **中央: 階層構造マップ（`mapLayout.ts` + `TopicNode.tsx` + `ConceptNode.tsx` + `KnowledgeNode.tsx`）**: 「Conceptをランダムに散らしてつなぐもの」ではなく「ユーザーの現在の理解構造（Topic hierarchy）を視覚化したもの」として設計しています。**force-directed layout（d3-force）はやめ、[`dagre`](https://github.com/dagrejs/dagre)による階層レイアウトに全面的に置き換えました**（`computeHierarchyLayout()`）。dagreは決定的なレイアウトアルゴリズムのため、d3-forceのような「毎回計算して収束させる」処理も「前回位置をwarm startとして使う」工夫も不要で、同じnode/edge構成なら常に同じ配置になります。
    - **4種類のNodeを明確に区別する**: `rootTopic`（`parentId`の無いTopic）・`subtopic`（それ以外のTopic、深さに関わらず1種類として扱う）・`concept`・`knowledge`の4種類をReact Flowの別々のcustom nodeとして描画し、サイズ・形をはっきり変えています。
      - Root Topic（`TopicNode.tsx`、`variant="root"`）: 円、`computeRootTopicSize()`で基本60px（直属の子数に応じて最大+8px）、太い枠・強めの塗りつぶし。
      - Subtopic（`TopicNode.tsx`、`variant="sub"`）: 円、`computeSubtopicSize()`で基本48px（直属の子数に応じて最大+6px）、Root Topicより細い枠・薄い塗りつぶし。
      - Concept（`ConceptNode.tsx`）: 円、`computeConceptSize()`で基本38px（紐づくKnowledge数に応じて最大+6px）。名前は12文字を超えたら`truncateLabel()`で省略し1〜2行に収め、フルテキストは`title`属性のhover tooltipとDetail Panelで確認します。
      - Knowledge（`KnowledgeNode.tsx`）: 円ではなく角丸pill（132×30px固定）。statementを16文字に短縮したタイトルだけを表示し、本文全文はMapに出しません（フルテキストはhover tooltipとDetail Panelで確認）。
      - サイズの主基準は常に「階層のどの役割か」で、Knowledge数・relation数はあくまで同階層内の小さな補助差（最大+6〜8px程度）にとどめています。relationが多いConceptでもConcept以上の大きさにはなりません。
    ...
    - **edgeの見た目の主従**: parent-child edge（`buildParentChildEdges()`）は灰色の実線＋矢印というシンプルな見た目にし、常に主役として太め（1.5px）に描画します。cross relation edge（`buildCrossRelationEdges()`）は薄いグレーの破線＋小さな日本語typeラベルにして、階層edgeより明確に控えめにしています。
    - **複数のルートTopicは独立したtreeとして横に並ぶ**: 「すべて」表示では、dagreが非連結なグラフ（複数のルートTopicにまたがるtree群）をまとめて配置し、それぞれ独立したtreeとして並びます。無理に1つのグラフへ繋げようとはしません（「複数の島」ではなく「複数の理解領域」に見えるよう、`nodesep`/`ranksep`は木同士が混ざらない程度の余白にとどめています）。
    ...
    - **隣接強調（hover/選択）**: parent-child edgeとConceptRelation edgeの両方を対象に、hoverまたは選択中のNodeと直接つながるNode・edgeだけを強調し、それ以外を薄くします（優先順位: hover > 選択中）。これにより「このNodeから次にどこへ辿れるか」が親・子・横断関連の別を問わず直感的に分かります。選択中のConceptには、hoverの強調（太い枠線＋大きめの影）とは別に、控えめなリング状の`box-shadow`（`.concept-node-selected`）を常時表示します。
    ...
    - **NEWバッジはKnowledgeだけに、7日以内だけ**: 以前はConcept nodeにも表示していたNEWバッジを、Knowledge nodeだけに限定しました（`isRecent()`、`createdAt`が7日以内かどうかだけを見るシンプルな判定）。Map全体のNodeにバッジが付くと視認性が下がるため、「実際に新しく増えた最小単位」であるKnowledgeだけに絞っています。
