...
Diggerは、ユーザーが理解した内容（KnowledgeやConcept）を「自分の理解が少しずつ育っている」と感じられる形で見せることを目指しています。そのための一つの見せ方が、それらをテーマごとの階層（トピック）に分けて表示することです。分類対象の項目はKnowledge（具体的な理解内容）の場合もConcept（MI6・政策金利のような理解対象そのもの）の場合もありますが、扱い方は同じです。

役割:
- 与えられた項目（concept/statement）それぞれに、1〜3階層のトピックパス（大分類→中分類→小分類）を付与してください。
...
- Diggerのトピック分類の目的は、学術的に厳密なタクソノミーを作ることではなく、**このユーザーの現時点の理解**をざっと眺めたときに「まとまり」を感じられるようにすることです。「一般的に正しい分類」を作ることが目的ではないので、同じ概念でもユーザーごと・そのときの理解状況ごとにトピック配置が変わって構いません。
- 新しいトピックを増やすこと自体を目的にしないでください。まず、既存のトピック（特にトップレベル）へ配置できないかを検討し、無理なく収まるならそちらを優先してください。
...
- トップレベルトピック（パスの1階層目）は粗く保ってください。「経済」「安全保障」「情報・インテリジェンス」「国際情勢」のように、ある程度広いテーマ領域を指す名前にし、細かい違いはsubtopicやconcept（分類対象の項目そのもの）側で表現してください。
- 新しいトップレベルトピックを作る前に、既存のトピックパス一覧の1階層目と意味的に近いものがないか必ず確認してください。近いものがあれば、新設せずその既存トップレベルトピックの下にサブトピックとして追加してください。
- 同じ意味領域を指すトップレベルトピックが既存一覧に複数ある場合（表記揺れや過去の分類のブレなど）は、それらのうち最も代表的な1つの名前に統合し、新しく分類する項目にはその統合後の名前を使ってください。
- このトピック分類は固定の正解ではありません。ユーザーの理解が進むにつれて、将来的にトピックの親子関係や統合が再編される前提で、今の情報から無理なく導ける自然な分類を答えてください。
...
function buildPrompt(input: KnowledgeTopicInput, extraInstruction?: string): string {
  ...
  const base = `以下の項目一覧それぞれに、トピックパスを付与してください。

# 分類対象の項目（KnowledgeまたはConcept）
${itemsList}
...
# 出力ルール
- assignments: 分類対象の項目全件について、idとpath（1〜3階層の配列。例:["経済","金融政策","政策金利"]）を1件ずつ返す。
