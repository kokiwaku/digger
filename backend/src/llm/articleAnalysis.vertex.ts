...
# 出力ルール
- summary: 3〜5文程度で記事内容を簡潔に説明する日本語の要約。見出しの言い換えだけにしないこと。
- whyItMatters: 「何が起きたか」の繰り返しではなく、なぜこの出来事・テーマを知る意味があるのかを説明する。ユーザーの日常・社会・経済・技術等への影響が記事から合理的に説明できる場合は含めてよいが、根拠がない場合は誇張しない。
- concepts: 記事を理解するために必要な前提知識を3〜6件。単なるキーワード列挙にしない（悪い例: 「東京」「9月」「会見」、良い例: 「政策金利」「中央銀行」「為替」）。importanceは"required"（これを知らないと記事の重要部分を理解しづらい）または"helpful"（知っていると理解が深まる）のいずれか。
- entities: 記事理解に重要な人物・組織・場所・出来事のみ。大量に列挙しない。
- connections: 記事のテーマと、より大きな社会・経済・技術等のテーマとの関係。記事本文から大きく逸脱した推測的な関連付けは避ける。
- deepDiveQuestions: ユーザーが次に質問すると理解が一段深まる問いを3〜5件。単純な事実確認だけでなく「なぜ？」「どういう仕組み？」「以前と何が違う？」「誰にどう影響する？」を優先する。
