...
画面ではこの解析結果を一度に全部表示せず、**Progressive Disclosure**（まず理解に必要な一段だけを見せ、興味に応じて掘れるUI）という方針で表示しています。最初に見えるのは「この記事で大事なのは（要約3文＋なぜ重要かの一言）」「まず知っておくこと（必須の前提知識2〜3件、各カードは1文だけ）」「次に掘るなら（おすすめの問い1件）」「自由入力欄」のみで、詳細な説明・関連テーマ・関連人物や組織・残りの前提知識/問いは「もう少し詳しく見る」「他の質問を見る」で展開する形です（詳細は[`frontend/README.md`](frontend/README.md)を参照）。
