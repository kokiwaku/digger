...
- **マップタブ**: 「全部を一目で理解する画面」ではなく「気になるConceptから、理解のつながりを辿る画面」を目指し、Topic/Concept/ConceptRelationモデル（後述）ベースのUI（検索／Topic・階層フィルタ／中央: Concept Map／右: Concept詳細）で構成しています（[reactflow](https://reactflow.dev/)を使用）。中央のMapはConceptだけをnode（名前と、あれば件数バッジのみのシンプルな表示、サイズは紐づくKnowledge数だけで3段階に決まる）、ConceptRelationをedgeとして表示し、Topicはグラフ上のnodeにはせず各クラスタの上に浮かぶ控えめな文字ラベル（クリックでそのクラスタに絞り込み）として表示します。node配置は[d3-force](https://github.com/d3/d3-force)のforce-directed layoutで計算し、「関連するConceptは近く、同じTopicのConceptは自然に集まる」有機的でコンパクトな配置にしています（力学シミュレーションは一度だけ計算して止め、continuous animationにはしません）。Conceptをクリックすると直接つながる（1-hopの）Concept・edgeだけが強調され、右側にそのConceptの詳細（紐づくKnowledge一覧・関連Concept・出典）が表示されます。関連Conceptや検索結果を選ぶと、Map上の該当Nodeへカメラが移動し選択状態になります（見たいものが決まっていれば検索、まだ決まっていなければMapを辿る、という役割分担）。「最近」「トピック」タブは引き続き旧`Knowledge.topicPath`モデルのまま無変更です（詳細は[`backend/README.md`](backend/README.md#topic--concept--conceptrelationモデルtopicts--conceptts--conceptrelationts--understandingstructurets)を参照）。
