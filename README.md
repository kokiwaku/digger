...
- **マップタブ**: 「Topic hierarchyを理解構造の背骨、ConceptRelationを理解の横方向の広がりとして扱う地図」です。Topic/Concept/ConceptRelationモデル（後述）を使い、UIは検索／Topicフィルタ／中央: マップ／右: Detail Panelで構成しています（[reactflow](https://reactflow.dev/)を使用）。node配置は[dagre](https://github.com/dagrejs/dagre)による階層レイアウトを骨格にしつつ、そのまま最終座標にはせず、決定的な（＝同じデータなら常に同じ結果になる）relaxed layout（少しの揺らぎ・ConceptRelationによる引き寄せ・重なり解消）を後段で適用しています。整列しすぎた「業務フロー図」に見えないよう、階層の読みやすさを保ったまま少し有機的な配置にする狙いです（詳細は[`frontend/README.md`](frontend/README.md)を参照）。**Map上のnodeはRoot Topic／Subtopic／Conceptの3種類のみ**（3段階のサイズではっきり区別、Nodeサイズは「relationやedgeがどれだけあるか」ではなく「理解構造上どの階層にいるか」を第一基準にします）。**Knowledgeはnodeとして表示せず**、Conceptをクリックした右Detail Panel（「自分が理解していること」）の中身として読みます（「Mapは地図を眺める場所、Detailは内容を読む場所」という役割分担）。**同一Conceptは常に1つのnodeとしてのみ表示され**、複数のTopicに属するConceptは、その全Topicから複数のhierarchy edgeを受け取ります（Conceptを複製してTreeを保つことはしません）。ConceptRelation（横断的なつながり）はレイアウトには関与せず、階層のparent-child edge（実線）より視覚的に控えめな補助edge（破線）として重ねるだけなので、**ConceptRelationが0件でもTopic hierarchyだけでMapとして成立**します。Nodeをクリックすると直接つながる（1-hopの）Node・edgeだけが強調され、右側にNode種別（Topic/Concept）に応じた詳細が表示されます。Topic・Concept・Knowledgeを横断した検索（Knowledgeの検索結果はそれが属するConceptへフォーカスします）や、Detail Panelからのnavigationを選ぶと、Map上の該当Nodeへカメラが移動し選択状態になります（見たいものが決まっていれば検索、まだ決まっていなければMapを辿る、という役割分担）。「最近」「トピック」タブは引き続き旧`Knowledge.topicPath`モデルのまま無変更です。**Topicは1つの親しか持てない設計のため、Knowledge Topic分類（LLM）がバッチごとに微妙に異なる経路で同じ名前のTopic（例:「SUV」）を複数作ってしまうことがありますが、「理解マップを更新」を押すたびに、同じ名前のTopic（NFKCで全角/半角も同一視）を1つに自動統合します**（詳細は[`backend/README.md`](backend/README.md#topic--concept--conceptrelationモデルtopicts--conceptts--conceptrelationts--understandingstructurets)を参照）。
