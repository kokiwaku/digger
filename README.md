...
- **マップタブ**: 「Topic hierarchyを理解構造の背骨、ConceptRelationを理解の横方向の広がりとして扱う地図」です。Topic/Concept/ConceptRelationモデル（後述）を使い、UIは検索／Topicフィルタ／中央: マップ／右: Detail Panelで構成しています（[reactflow](https://reactflow.dev/)を使用）。node配置は[dagre](https://github.com/dagrejs/dagre)による決定的なレイアウトです。**Map上のnodeはRoot Topic／Subtopic／Conceptの3種類のみ**（3段階のサイズではっきり区別、Nodeサイズは「relationやedgeがどれだけあるか」ではなく「理解構造上どの階層にいるか」を第一基準にします）。**Knowledgeはnodeとして表示せず**、Conceptをクリックした右Detail Panel（「自分が理解していること」）の中身として読みます（「Mapは地図を眺める場所、Detailは内容を読む場所」という役割分担）。**同一Conceptは常に1つのnodeとしてのみ表示され**、複数のTopicに属するConceptは、その全Topicから複数のhierarchy edgeを受け取ります（Conceptを複製してTreeを保つことはしません）。ConceptRelation（横断的なつながり）はレイアウトには関与せず、階層のparent-child edge（実線）より視覚的に控えめな補助edge（破線）として重ねるだけなので、**ConceptRelationが0件でもTopic hierarchyだけでMapとして成立**します。Nodeをクリックすると直接つながる（1-hopの）Node・edgeだけが強調され、右側にNode種別（Topic/Concept）に応じた詳細が表示されます。Topic・Concept・Knowledgeを横断した検索（Knowledgeの検索結果はそれが属するConceptへフォーカスします）や、Detail Panelからのnavigationを選ぶと、Map上の該当Nodeへカメラが移動し選択状態になります（見たいものが決まっていれば検索、まだ決まっていなければMapを辿る、という役割分担）。「最近」「トピック」タブは引き続き旧`Knowledge.topicPath`モデルのまま無変更です（詳細は[`backend/README.md`](backend/README.md#topic--concept--conceptrelationモデルtopicts--conceptts--conceptrelationts--understandingstructurets)を参照）。
